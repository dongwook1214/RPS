...
class GameContract {
    ...
    public async getGameInfo(): Promise<GameInfo> {
        ...
        return { game_id, stake, ongoing, period, player1, player2, winner, address: this.contractAddress };
    }
    ...
    public async getOpponentUserProcessState(): Promise<UserProcessState> {
        ...
        if (player1 == address) {
            ...
            return { c: (await _c).toString(), m: (await _m).toString(), isOpen: await _isOpen };
        } else {
